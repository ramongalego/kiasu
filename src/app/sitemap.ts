...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const listEntries: MetadataRoute.Sitemap = publicLists.map((list) => ({
    url: `https://www.kiasu.co/share/${list.id}`,
    lastModified: list.updatedAt,
  }));

  const userEntries: MetadataRoute.Sitemap = users.map((user) => ({
    url: `https://www.kiasu.co/user/${user.username}`,
    lastModified: user.updatedAt,
  }));

  return [
    { url: 'https://www.kiasu.co', lastModified: new Date(), priority: 1 },
    {
      url: 'https://www.kiasu.co/discovery',
